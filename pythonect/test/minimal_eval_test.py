...
class TestPythonect(unittest.TestCase):
    ...
    def test_literal_float(self):
        ...
        self.assertEqual(internal.eval.eval('1.0', {}, {}), 1)

    def test_literal_int(self):

        self.assertEqual(internal.eval.eval('1', {}, {}), 1)
    ...
    def test_python_expr_str_3(self):
        ...
        self.assertEqual(internal.eval.eval('"Hello \'W\'orld"', {}, {}), "Hello 'W'orld")
    ...
    def test_python_false_expr_literal_neq_literal(self):
        ...
        self.assertEqual(internal.eval.eval('1 == 0', {}, {}), False)

    def test_python_true_expr_underscore_eq_underscore(self):

        self.assertEqual(internal.eval.eval('_ == _', {}, {}), self.input)

    def test_python_false_expr_underscore_neq_repr_underscore(self):

        self.assertEqual(internal.eval.eval('_ == repr(_)', {}, {}), False)

    def test_literal_int_async_none(self):

        self.assertEqual(internal.eval.eval('1 -> None', {}, {}), 1)
    ...
    def test_literal_int_async_dict(self):
        ...
        self.assertEqual(internal.eval.eval('1 -> {1: "One", 2: "Two"}', {}, {}), "One")
    ...
    def test_literal_str_async_autoload(self):
        ...
        self.assertEqual(internal.eval.eval('"Hello world" -> string.split', {}, {}), ["Hello", "world"])

    def test_literal_str_sync_autoload(self):

        self.assertItemsEqual(internal.eval.eval('"Hello world" | string.split', {}, {}), ["Hello", "world"])

    def test_literal_int_async_literal_int(self):

        self.assertEqual(internal.eval.eval('1 -> 1', {}, {}), 1)
    ...
    def test_literal_int_async_literal_array_int_float(self):
        ...
        self.assertItemsEqual(internal.eval.eval('1 -> [int,float]', {}, {}), [1, 1.0])

    def test_literal_int_sync_literal_array_int_float(self):

        self.assertEqual(internal.eval.eval('1 | [int,float]', {}, {}), [1, 1.0])

    def test_python_stmt_assignment_async_literal_int(self):

        self.assertEqual(internal.eval.eval('[x = 0] -> 1', {}, {}), 1)
    ...
    def test_python_stmt_assignment_sync_variable(self):
        ...
        self.assertEqual(internal.eval.eval('[x = 0] | x', {}, {}), 0)
    ...
    def test_literal_array_int_int_sync_none(self):
        ...
        self.assertEqual(internal.eval.eval('[1, 2] | None', {}, {}), [1, 2])

    def test_literal_array_int_int_async_none(self):

        self.assertItemsEqual(internal.eval.eval('[1, 2] -> None', {}, {}), [1, 2])

    def test_literal_array_int_int_sync_dict(self):

        self.assertEqual(internal.eval.eval('[1, 2] | {1: "One", 2: "Two"}', {}, {}), ["One", "Two"])

    def test_literal_array_int_int_async_dict(self):

        self.assertItemsEqual(internal.eval.eval('[1, 2] -> {1: "One", 2: "Two"}', {}, {}), ["One", "Two"])

    def test_literal_array_int_str_async_none(self):

        self.assertItemsEqual(internal.eval.eval('[1, "Hello"] -> None', {}, {}), [1, "Hello"])

    def test_literal_array_int_str_sync_none(self):

        self.assertEqual(internal.eval.eval('[1, "Hello"] | None', {}, {}), [1, "Hello"])

    def test_literal_array_int_str_async_int(self):

        self.assertItemsEqual(internal.eval.eval('[1, "Hello"] -> 1', {}, {}), [1, 1])

    def test_literal_array_int_str_sync_int(self):

        self.assertEqual(internal.eval.eval('[1, "Hello"] | 1', {}, {}), [1, 1])

    def test_literal_array_int_int_sync_literal_int(self):

        self.assertEqual(internal.eval.eval('[1, 2] | 1', {}, {}), [1, 1])
    ...
    def test_literal_array_int_int_sync_literal_array_int_int(self):
        ...
        self.assertEqual(internal.eval.eval('[1, 2] | [3, 4]', {}, {}), [3, 4, 3, 4])

    def test_literal_array_int_int_async_literal_array_int_int(self):

        self.assertItemsEqual(internal.eval.eval('[1, 2] -> [3, 4]', {}, {}), [3, 3, 4, 4])

    def test_literal_int_async_stmt_single_return_value_function_async_single_return_value_function(self):

        self.assertEqual(internal.eval.eval('1 -> def foobar(x): return x+1 -> foobar', {}, {}), 2)
    ...
    def test_literal_int_async_stmt_multiple_return_value_function_async_multiple_return_value_function(self):
        ...
        self.assertItemsEqual(internal.eval.eval('1 -> def foobar(x): return [x,x+1] -> foobar', {}, {}), [1, 2])

    def test_literal_int_async_stmt_multiple_return_value_function_sync_multiple_return_value_function(self):

        self.assertEqual(internal.eval.eval('1 -> def foobar(x): return [x,x+1] | foobar', {}, {}), [1, 2])
    ...
    def test_literal_int_async_stmt_generator_return_value_function_async_generator_return_value_function(self):
        ...
        self.assertItemsEqual(internal.eval.eval('1 -> def foobar(x): yield x; yield x+1 -> foobar', {}, {}), [1, 2])

    def test_literal_int_async_stmt_generator_return_value_function_sync_generator_return_value_function(self):

        self.assertEqual(internal.eval.eval('1 -> def foobar(x): yield x; yield x+1 | foobar', {}, {}), [1, 2])
    ...
    def test_singlethread_program_async(self):
        ...
        self.assertEqual(internal.eval.eval('import threading -> x = threading.current_thread().name -> y = threading.current_thread().name -> x == y', {}, {}), None)
    ...
    def test_multithread_program_async(self):
        ...
        r_array = internal.eval.eval('import threading -> [threading.current_thread().name, threading.current_thread().name]', {}, {})
    ...
    def test_multiprocess_program_async(self):
        ...
        self.assertEqual(internal.eval.eval('import multiprocessing -> start_pid = multiprocessing.current_process().pid -> start_pid -> str & -> current_pid = multiprocessing.current_process().pid -> 1 -> current_pid != start_pid', {}, {}), 1)
    ...
    def test_pseudo_none_const_as_url(self):
        ...
        self.assertEqual(internal.eval.eval('def foobar(x): return x+1 -> 1 -> foobar@None', {}, {}), 2)
    ...
    def test_pseudo_none_value_fcn_return_value_as_url(self):
        ...
        self.assertEqual(internal.eval.eval('def ret_none(): return None -> def foobar(x): return x+1 -> 1 -> foobar@ret_none()', {}, {}), 2)
    ...
    def test_print_like_statement(self):
        ...
        self.assertItemsEqual(internal.eval.eval('range(1,10) -> print("Thread A")', {}, {}), [1, 2, 3, 4, 5, 6, 7, 8, 9])
    ...
    def test_multiple_stateful_x_eq_5_statement(self):
        ...
        internal.eval.eval('xrange(1, 10) -> x = _', globals_, locals_)
    ...
    def test_stateful_x_eq_5_statement(self):
        ...
        internal.eval.eval('x = 5', globals_, locals_)

        self.assertEqual(internal.eval.eval('1 -> [x == 5]', globals_, locals_), 1)
    ...
    def test_typeerror_exception_not_due_to_eval(self):
        ...
        self.assertEqual(internal.eval.eval('1 -> socket.socket(socket.AF_INET, socket.SOCK_STREAM) -> _.connect("A","B")', {}, {}), False)
    ...
    def test_multi_processing_and_multi_threading(self):
        ...
        self.assertEqual(internal.eval.eval('"Hello, world" -> [print, print &]', {}, {}), ['Hello, world', 'Hello, world'])
